...
{
PHX : PHY
LDA $0A5C : CMP #$EB87 : BPL ..out	;quit drawing the aura if Samus is dead/others
if !WaveDash_Mccad == 1 && !OverrideAura == 1
	LDA !VarWState : BNE ..out ;if wave dash state != 0 (0 means not dashing), abort outline
endif
if !criticalAura == 1
	LDA $0A6A : BNE ++
endif
...
if !criticalAura == 1 || !chargeAura == 1
	...
	+
PLA
STA $0372,x			;hijacked instruction	;920372
RTS
!free81 #= pc()
